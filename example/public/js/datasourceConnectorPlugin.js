...
dataSourceConnectorPlugin.onDataSend = function (method,url, payload, data, status) {
  var container = document.getElementById('log');
  var row = container.insertBefore(document.createElement('tr'), container.firstChild);
  row.innerHTML="<td>" + method + "</td><td>"+ url +"</td><td>" + JSON.stringify(payload) + "</td><td>" + JSON.stringify(data) + "</td><td>" + status + "</td>";
}
/**
...
dataSourceConnectorPlugin.prototype._sendData = function(
  ...
  xhr.onreadystatechange = function(response) {
    ...
    if (xhr.readyState == 4) {
      ...
      dataSourceConnectorPlugin.onDataSend('POST',controllerUrl + "/" + endpoint, payload, data, status)
    }
  }
...
dataSourceConnectorPlugin._getData = function(
  ...
  xhr.onreadystatechange = function() {
    ...
    if (xhr.readyState == 4) {
      ...
      dataSourceConnectorPlugin.onDataSend('GET',url, {}, data, status)
    }
  };
  xhr.send();
};
